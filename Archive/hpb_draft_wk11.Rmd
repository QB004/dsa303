...
school_obesity_map <- tm_shape(obesity_data_sf_2023_OSO, name = "Overweight prevalence") +
  tm_symbols(
    ...
    fill.legend = tm_legend(
      title = "Overweight prevalence by school", orientation = "landscape",
...
We recommend skipping to the last sub-section to examine the autocorrelation of our selected spatial weights matrix, and our results from the spatial clustering analysis of childhood obesity in Singapore
...
write_gal(moe_clusters_pri, "galFiles/moe_clusters_pri.gal")

moe_clusters_sec <- obesity_sec_sch %>%
  st_drop_geometry() %>%
  select(PT_ID, SCHOOL, CLUSTER)
write_gal(moe_clusters_sec, "galFiles/moe_clusters_sec.gal")
...
```{r}
#### Correlogram (does not work, some sub-graphs have 2 or less neighbours)
...
nb_pval_pri %>% as_tibble()
nb_pval_sec %>% as_tibble()
```
...
*Positive spatial autocorrelation for Primary Schools*
...
*Weaker evidence of autocorrelation for Secondary Schools*
...
Local Getis-Ord's G, returned as a z-score, suggests a high-high prevalence clusters or hot spot for positive z-values, and low-low prevalence clusters or cold spots for negative z-values.
...
pri_map <- 
  ...
  tm_symbols(fill = "LMORANZ", size = 0.5) + 
  tm_title("Local Moran's I Z-Statistic")
...
pri_map <-
  tm_shape(nb2lines(soi_nb_pri, coords = st_geometry(obesity_pri_sch))) +
  tm_lines(lwd = 2) + 
  tm_shape(obesity_pri_sch) + tm_symbols(fill = "GO_STAT", size = 0.5) +
  tm_title("Local Getis-Ord's G statistics")
...
sec_map <- 
  tm_shape(nb2lines(soi_nb_sec, coords = st_geometry(obesity_sec_sch))) +
  tm_lines(lwd = 2) + 
  tm_shape(obesity_sec_sch) + tm_symbols(fill = "GO_STAT", size = 0.5) +
  tm_title("Local Getis-Ord's G statistics")
...
Based on Local Moran's I (at the 1% significance level) and Local Getis-Ord's G (z-value > 0), the following primary schools (and their respective SOI neighbours) were identified as High-High overweight prevalence clusters:
...
No secondary schools were identified as High-High overweight prevalence clusters.

...
jc_pri <- joincount.multi(obesity_pri_sch$HIGH_PREVALENCE, soi_lw_B_pri) %>% as.data.frame()
jc_pri$Category <- rownames(jc_pri)
jc_pri[, c("Category", "Joincount", "Expected", "Variance", "z-value")] %>% gt() %>% 
  tab_header(
...
jc_sec <- joincount.multi(obesity_sec_sch$HIGH_PREVALENCE, soi_lw_B_sec) %>% as.data.frame()
jc_sec$Category <- rownames(jc_pri)
jc_sec[, c("Category", "Joincount", "Expected", "Variance", "z-value")] %>% gt() %>% 
  tab_header(
    title = "Join count analysis (SOI)",
    subtitle = "Secondary schools"
  ) %>%

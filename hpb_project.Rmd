...
Strong evidence of spatial clustering of obesity is observed under most spatial weight matrices.

The Moran’s I test indicates significant positive spatial autocorrelation for primary schools under most configurations (e.g., Delaunay triangulation, Sphere of Influence, and k-nearest neighbours with k = 2 or 3).

Geary’s C test further supports the presence of spatial dependence, reinforcing the finding of clustering.

The Getis-Ord’s G test results are less consistent, though some configurations (e.g., Delaunay triangulation and k = 2, 3 nearest neighbours) suggest significant clustering of high-obesity schools.

Secondary Schools:

The evidence for spatial clustering is weaker for secondary schools.

Moran’s I and Geary’s C tests show insignificant p-values for many spatial weight matrices, particularly for planning areas and MOE clusters.

Significant spatial autocorrelation is only detected under some configurations, such as Delaunay triangulation, Sphere of Influence, and k = 3 nearest neighbours.

Getis-Ord’s G test does not strongly indicate clustering, except in a few cases.

Conclusion:
There is strong spatial clustering of obesity among primary schools across Singapore, suggesting that local factors and neighbourhood effects significantly influence childhood obesity rates. In contrast, secondary schools exhibit weaker and less consistent spatial clustering, implying that obesity prevalence may be more influenced by non-spatial factors such as school policies, socio-economic differences, or lifestyle choices.

From a policy perspective, targeted interventions in primary school clusters could be more effective in leveraging spillover effects. The choice of spatial weight matrix influences the observed clustering patterns, but results are broadly consistent across graph-based and k-nearest neighbour methods. Future studies could refine spatial definitions to improve accuracy in detecting secondary school clusters.

For social neighbours (MOE clusters), we find that 

Examining graph neighbours, Delauney triangulation neighbours tend to have links that are relatively long. Sphere of Influence graphs help to thin the triangulation and remove long links (e.g. schools along the borders of Singapore were indicated as neighbours even when they were very far apart)
...
However, due to the arbitrary neighbour links introduced by Delauney triangulation , we determined that Sphere of Influence was the better choice to model neighbour links instead.

SOI neighbours are symmetric by design – if i is a neighbour of j, then j is a neighbour of.

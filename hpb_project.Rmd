...
del_nb_pri <- tri2nb(st_coordinates(obesity_pri_sch_jitter), 
                     row.names = obesity_pri_sch_jitter$PT_ID) 
pri_map <- tm_shape(nb2lines(del_nb_pri, coords = st_geometry(obesity_pri_sch_jitter))) +
  tm_lines(lwd = 2) + pri_sch_map

# by Secondary schools
obesity_sec_sch_jitter <- st_jitter(obesity_sec_sch) 
del_nb_sec <- tri2nb(st_coordinates(obesity_sec_sch_jitter), 
                     row.names = obesity_sec_sch_jitter$PT_ID) 
sec_map <- tm_shape(nb2lines(del_nb_sec, coords = st_geometry(obesity_sec_sch_jitter))) +
...
if (require(dbscan, quietly=TRUE)) {
  soi_nb_pri <- graph2nb(soi.graph(del_nb_pri, st_geometry(obesity_pri_sch_jitter)))
} else {
  soi_nb = NULL
}
...
# by Secondary schools
if (require(dbscan, quietly=TRUE)) {
  soi_nb_sec <- graph2nb(soi.graph(del_nb_sec, st_geometry(obesity_sec_sch_jitter)))
} else {
  soi_nb = NULL
}
...
cca_planning_avg_map <- ura_planning_area_map + cca_planning_avg %>% 
  ...
  tm_title("Average Sports CCA Ratio by URA Planning Area") + 
  tm_facets_wrap(by = "LEVEL")
...
# Aggregating overweight counts by URA planning areas
cca_ura_planning_areas <- st_join(ura_planning_areas, obesity_data_sf) %>%
  filter(YEAR == 2023, CATEGORY == "OSO") %>% 
  group_by(PLN_AREA_N, LEVEL) %>% 
  summarise(
    PREVALENCE_AGGREGATE = sum(COUNT, na.rm = T) / sum(ENROLMENT, na.rm = T),

...
library(sf); library(spData); library(tmap); library(tidyverse)
...
sg_crs <- st_crs(sg_pa) # no epsg given, so we use proj4string instead

# Loading cleaned obesity data from HPB and giving spatial awareness
obesity_data <- 
  # null values converted from #N/A to NA
...
isTRUE(all.equal(st_crs(sg_pa), st_crs(obesity_data), st_crs(hmsp_data_sf))) # CRS must match in order for data to be plotted correctly
```

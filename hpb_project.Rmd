...
school_obesity_map <- tm_shape(obesity_data_sf_2023_OSO, name = "Overweight prevalence") +
  tm_symbols(
    ...
    fill.legend = tm_legend(
      ...
      position = c("BOTTOM", "LEFT"), item.height = 0.5, item.width = 2.1)
    )
...
We aggregate overweight counts and school enrolment data by the different administrative regions, and fill in the regions to get visualise overweight prevalence by region.
...
sg_map + moe_zones_map + tm_legend_hide() + obesity_moe_zones %>% 
  tm_shape(name = "Overweight prevalence") +
  tm_polygons(
    fill = "PREVALENCE_AGGREGATE",
    fill_alpha = 0.95,
...
sg_map + moe_clusters_map + obesity_moe_clusters %>% 
  tm_shape(name = "Overweight prevalence") +
  tm_polygons(
    fill = "PREVALENCE_AGGREGATE",
    fill_alpha = 0.95,
...
pri_map <- ura_regions_map + ura_regions_nb_line_pri %>% tm_shape() + tm_lines(lwd = 2) + 
...
sec_map <- ura_regions_map + ura_regions_nb_line_sec %>% tm_shape() + tm_lines(lwd = 2) + 
...
pri_map <- sg_map + moe_clusters_map + 
  ...
  obesity_pri_sch %>% tm_shape() + tm_symbols(size = 0.4) +
...
sec_map <- sg_map + moe_clusters_map +
  ...
  obesity_sec_sch %>% tm_shape() + tm_symbols(size = 0.4) +
...
pri_map <- sg_map + tm_shape(nb2lines(del_nb_pri, coords = st_geometry(obesity_pri_sch))) +
  tm_lines(lwd = 0.5) + obesity_pri_sch %>% tm_shape() + tm_symbols(size = 0.4) +
...
sec_map <- sg_map + tm_shape(nb2lines(del_nb_sec, coords = st_geometry(obesity_sec_sch))) +
  tm_lines(lwd = 0.5) + obesity_sec_sch %>% tm_shape() + tm_symbols(size = 0.4) +
...
pri_map <- sg_map + tm_shape(nb2lines(soi_nb_pri, coords = st_geometry(obesity_pri_sch))) +
  tm_lines(lwd = 2) + obesity_pri_sch %>% tm_shape() + tm_symbols(size = 0.4) +
...
sec_map <- sg_map + tm_shape(nb2lines(soi_nb_sec, coords = st_geometry(obesity_sec_sch))) +
  tm_lines(lwd = 2) + obesity_sec_sch %>% tm_shape() + tm_symbols(size = 0.4) +
...
Based on the spatial autocorrelation tests conducted using Moran’s I, Geary’s C, and Getis-Ord’s G statistics, we can conclude the following:
...
The Moran’s I test indicates highly significant (p-values < 0.01) positive spatial autocorrelation for primary schools under all configurations except for social neighbours (MOE clusters). We can reject the null hypothesis at the 1% significance level in favour of the presence of positive spatial autocorrelation. 

Geary’s C test further supports the presence of spatial dependence at the 1% significance level, reinforcing the finding of clustering (with the exception of social neighbours and k = 1 nearest neighbours). Getis-Ord’s G test results are less consistent, though some configurations (Delaunay triangulation, Sphere of Influence, and k = 2, 3 nearest neighbours) continue to suggest moderately significant (p-values < 0.10) clustering of obesity in Primary schools.
...
The Moran’s I test indicates highly significant (p-values < 0.01) positive spatial autocorrelation for primary schools under all configurations except for contiguous neighbours (Planning Areas) and social neighbours (MOE clusters).

Geary’s C test further supports the presence of spatial dependence at the 1% significance level, but with the exception of contiguous neighbours, social neighbours and k = 2 nearest neighbours. However, Getis-Ord’s G test fails to reject the null hypothesis at the 10% significance level for all spatial weight matrices.
...
Of the spatial weights matrices examined, the graph-based and distance-based neighbours had the most consistent results for the global autocorrelation tests used. Focusing on the graph-based neighbours, Delauney triangulation had neighbour links that were relatively far apart. Sphere of Influence graphs had an advantage in thinning down the triangulation and removing long links, ensuring that schools along the edges of Singapore are not indicated as neighbours when they were in fact quite far apart geographically. Sphere of Influence and k = 3 nearest neighbours also share similar neighbour to neighbour links and similar average number of links (~3). 

```{r}
# Visualisation of graph-based and distanced-base neighbours
par(mfrow = c(1,3))
plot(st_union(sg_ura_boundaries), border="grey60", axes=F,  main="Delauney triangulation neighbours")
plot(del_nb_pri, st_coordinates(obesity_pri_sch), pch=19, cex=0.6, add=T)
plot(st_union(sg_ura_boundaries), border="grey60", axes=F,  main="SOI neighbours")
plot(soi_nb_pri, st_coordinates(obesity_pri_sch), pch=19, cex=0.6, add=T)
plot(st_union(sg_ura_boundaries), border="grey60", axes=F, main="k nearest neighbours")
plot(knn3_nb_pri, st_coordinates(obesity_pri_sch), col="red4", pch=19, cex=0.6, add=T)
plot(knn2_nb_pri, st_coordinates(obesity_pri_sch), col="red", pch=19, cex=0.6, add=T)
plot(knn1_nb_pri, st_coordinates(obesity_pri_sch), col="pink", pch=19, cex=0.6, add=T)
legend("bottomright", c("k=1", "k=2", "k=3"), col=c("pink", "red", "red4"), lty=1)
par(mfrow = c(1,1))
```

Additionally, graphs-based spatial weights matrices like SOI neighbours are [symmetric](https://cran.r-project.org/web/packages/spdep/vignettes/nb.html#Creating_Graph-Based_Neighbours) by design, that is if i is a neighbour of j, then j is a neighbour of i. K nearest neighbours spatial weights matrices may not be symmetric as one school's nearest neighbour may have a different nearest neighbour. The symmetric property for SOI may be more desirable to model spillover effects.

As such, we determined that Sphere of Influence was the best choice to model neighbour links.

We use local autocorrelation statistics to identify spatial clusters:
...
obesity_pri_sch$HIGH_PREVALENCE <- 
  as.factor(obesity_pri_sch$PREVALENCE > OVERWEIGHT_CUTOFF)
pri_map <- sg_map + obesity_pri_sch %>% 
  tm_shape(name = "Overweight prevalence") + 
  tm_symbols(
    fill = "HIGH_PREVALENCE", 
    size = "PREVALENCE", size.scale = tm_scale_continuous(values.scale = 0.8),
...
obesity_sec_sch$HIGH_PREVALENCE <- 
  as.factor(obesity_sec_sch$PREVALENCE > OVERWEIGHT_CUTOFF)
sec_map <- sg_map + obesity_sec_sch %>% 
  tm_shape(name = "Overweight prevalence") + 
  tm_symbols(
    fill = "HIGH_PREVALENCE", 
    size = "PREVALENCE", size.scale = tm_scale_continuous(values.scale = 0.8),
...
hh_pri %>% select(-PT_ID) %>% st_drop_geometry() 
...
hh_pri_neighbours %>% 
  ...
  obesity_pri_sch %>% filter(PT_ID %in% hh_pri_neighbours$j_ID) %>% tm_shape() + tm_symbols(
    fill = "PREVALENCE", size = 0.5, 
    fill.scale = tm_scale(breaks = seq(0, 0.3, 0.05), values = "brewer.yl_or_rd")
    ) + 
  hh_pri %>% tm_shape() + tm_symbols(
    fill = "PREVALENCE", size = 0.5, col = "green", lwd = 2, fill.legend = tm_legend(show = F),
    fill.scale = tm_scale(breaks = seq(0, 0.3, 0.05), values = "brewer.yl_or_rd")
    ) + 
  tm_legend(bg.color = "white") +
...
Targeting these 5 identified primary schools for obesity reduction could have potential spillover effects for obesity reduction in neighbouring schools in the cluster as well. 
...
ll_pri <- obesity_pri_sch %>%
  ...
  select(PT_ID, SCHOOL, PREVALENCE, ZONE, CLUSTER, LMORANZ, GO_STAT) %>%
  arrange(desc(LMORANZ))
ll_pri %>% select(-PT_ID) %>% st_drop_geometry() 
...
pri_map <- ll_pri_neighbours %>% 
  # select(PT_ID, SCHOOL, PREVALENCE, ZONE, CLUSTER, LMORANZ, GO_STAT) %>% 
  tm_shape() + tm_lines(lwd = 2) +
  obesity_pri_sch %>% filter(PT_ID %in% ll_pri_neighbours$j_ID) %>% tm_shape() + tm_symbols(
    fill = "PREVALENCE", size = 0.5, 
    fill.scale = tm_scale(breaks = seq(0, 0.3, 0.05), values = "brewer.yl_or_rd")
    ) + 
...
ll_sec <- obesity_sec_sch %>%
  ...
  select(PT_ID, SCHOOL, PREVALENCE, ZONE, CLUSTER, LMORANZ, GO_STAT) %>%
  arrange(desc(LMORANZ))
ll_sec %>% select(-PT_ID) %>% st_drop_geometry() 
...
sec_map <- ll_sec_neighbours %>% 
  # select(PT_ID, SCHOOL, PREVALENCE, ZONE, CLUSTER, LMORANZ, GO_STAT) %>% 
  tm_shape() + tm_lines(lwd = 2) +
  obesity_sec_sch %>% filter(PT_ID %in% ll_sec_neighbours$j_ID) %>% tm_shape() + tm_symbols(
    fill = "PREVALENCE", size = 0.5, 
    fill.scale = tm_scale(breaks = seq(0, 0.3, 0.05), values = "brewer.yl_or_rd")
    ) + 
...
Based on Local Moran's I (at the 1% significance level) and Local Getis-Ord's G (z-value < 0) (we filter for LMORANZ >= 2.576, GO_STAT < 0 to identify high-high clusters and schools with high positive spatial autocorrelation), the following primary schools (circled in green in the visualisation), and their respective SOI neighbours, were identified as Low-Low overweight prevalence clusters:
...
Observing these 3 identified primary schools and 2 identified secondary schools for upticks in obesity would be useful to implement obesity mitigation measures on neighbouring schools in the clusters, which may receive spillover effects from obesity increases. 
...
Next, we consider the Education and Income statistics from [Singapore Census of Population 2020](https://data.gov.sg/datasets/d_2d6793de474551149c438ba349a108fd/view).
...
print(sec_food_map)
```
As a proxy for food options, we use McDonald's outlets, 7-11 outlets and Hawker Centres as these are frequented by youths and are easily accessible. 

Based on the descriptive maps, we can observe a large number of food outlets concentrated towards the south. The converse is true for schools located in the North, where food options range from 0 to 5. 

## Spatial Descriptive Summary Measures of Education Years
```{r}
#Spatial analysis - Secondary school
st_crs(obesity_sec_food)$proj 
...
(sec_food_map <- ura_planning_areas_map +
...
#Plot the summary measures 
(pri_food_map <- ura_planning_areas_map +
...
```
We can observe that the weighted SDE for Primary Schools is skewed Southwest compared to the SDE. Such is the case for Secondary Schools as well. This can be seen through the prevalence of a large number of food outlets concentrated in the Bukit Merah and Tanglin areas.
...
```
We also explore the distance to the nearest food option to investigate the ease of accessibility for Primary and Secondary School students for meals outside of school. 

Based on the descriptive maps, we observe that schools in the central region have the largest distances to the nearest food option available. 
...
Aggregating the sports CCA ratio by URA planning area, we observe no generalised pattern of school sports participation across Singapore. 
...
pri_dus_map <- ura_planning_areas_map + 
  ...
  tm_layout(
    legend.position = c("BOTTOM", "RIGHT"),
    legend.title.size = 1.2,
    main.title = "C2 - Primary Schools and Dual Use Sports Facilities"
  )
...
sec_dus_map <- ura_planning_areas_map + 
  ...
  tm_layout(
    legend.position = c("BOTTOM", "RIGHT"),
    legend.title.size = 1.2,
    main.title = "C3 - Secondary Schools and Dual Use Sports Facilities"
  )
...
Such is the case for Telok Kurau Primary School, Seng Kang Primary School, Naval Base Primary School, St. Stephen's Primary School, Xinmin Primary School and Singapore Chinese Girls' School. The aforementioned schools had declining obesity levels prior to Covid. However, following the pandemic, these schools faced increasing levels of obesity which is a cause for concern.

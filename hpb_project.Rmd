...
jois_special_schount.multi(obesity_ura_planning_areas_pri$HIGH_PREVALENCE, 
                ura_planning_areas_lw_B_pri) 
...
sec_map <- ura_regions_map + obesity_ura_planning_areas_sec %>% 
  tm_shape(name = "Overweight prevalence") + tm_polygons(fill = "HIGH_PREVALENCE") +
  tm_title("Join count analysis")
jois_special_schount.multi(obesity_ura_planning_areas_sec$HIGH_PREVALENCE, 
                ura_planning_areas_lw_B_sec)
...
#   moe_clusters_nb_line_pri %>% tm_shape() + tm_lines(lwd = 2) + 
#   st_point_on_surface(st_geometry(obesity_moe_clusters_pri)) %>% tm_shape() + tm_symbols()
# 
# obesity_moe_clusters_sec <- obesity_moe_clusters %>% filter(LEVEL == "SECONDARY")
# moe_clusters_nb_sec <- 
...
#   moe_clusters_nb_line_sec %>% tm_shape() + tm_lines(lwd = 2) + 
#   st_point_on_surface(st_geometry(obesity_moe_clusters_sec)) %>% tm_shape() + tm_symbols()
# 
# tmap_arrange(pri_map, sec_map)
```
...
pri_map <- ura_regions_map + obesity_pri_sch %>% 
  ...
  tm_symbols(
    ...
    size.legend = tm_legend_hide()) +
  tm_title("Join count analysis")
jois_special_schount.multi(obesity_pri_sch$HIGH_PREVALENCE, 
...
sec_map <- ura_regions_map + obesity_sec_sch %>% 
  ...
  tm_symbols(
    ...
    size.legend = tm_legend_hide()) +
  tm_title("Join count analysis")
jois_special_schount.multi(obesity_sec_sch$HIGH_PREVALENCE, 
...
# is_special_schome distribution (Lucas)
...
census_merge <- ura_planning_areas %>%
  left_join(census) %>%
  drop_na("is_special_schome")

ura_planning_areas_map + tm_shape(census_merge) + 
  tm_polygons(fill="is_special_schome", 
              fill.scale = tm_scale(breaks = c(6925,8396,9532,11317,16197))
              ) +
  tm_title("Breakdown by Household is_special_schome")
...
# Spatial Descriptive Summary Measures of is_special_schome (Lucas)

```{r}
census_coords <- st_coordinates(st_point_on_surface(census_merge))
is_special_schome_mean <- calc_mnc(id = 1, points = census_coords)
is_special_schome_median <- calc_mdc(id = 2, points = census_coords)
is_special_schome_mean_median <- rbind(is_special_schome_mean$LOCATIONS, is_special_schome_median$LOCATIONS) %>% 
  data.frame() %>% 
  st_as_sf(coords = c("x", "y"), crs = sg_crs)
is_special_schome_mean_median$id = c("Weighted mean", "Weighted median")

is_special_schome_sdd <- calc_sdd(id = 3, points = census_coords)
is_special_schome_sdd_line <- is_special_schome_sdd$LOCATIONS %>% 
  st_as_sf(coords = c("x", "y"), crs = sg_crs) %>% 
  st_combine() %>% 
  st_cast("LINESTRING")

is_special_schome_sde <- calc_sde(id = 4, points = census_coords)
is_special_schome_sde_line <- is_special_schome_sde$LOCATIONS %>% 
  st_as_sf(coords = c("x", "y"), crs = sg_crs) %>% 
  st_combine() %>% 
  st_cast("LINESTRING")

is_special_schome_sde_weighted <- calc_sde(
  id = 5, calccentre = T, weighted = T, weights = census_merge$is_special_schome,
  points = census_coords)
is_special_schome_sde_weighted_line <- is_special_schome_sde_weighted$LOCATIONS %>% 
  st_as_sf(coords = c("x", "y"), crs = sg_crs) %>% 
  st_combine() %>% 
  st_cast("LINESTRING")

is_special_schome_map <- ura_planning_areas_map +
    tm_shape(census_merge, name = "is_special_schome") +
    tm_symbols(
        size = "is_special_schome",
        size.scale = tm_scale_continuous(values.scale = 0.8),
        size.legend = tm_legend_hide(),
        fill = "is_special_schome", 
        # a green-red diverging scale to indicate severity of overweight prevalence
        fill.scale = tm_scale_continuous(values = "brewer.yl_or_rd"), 
        fill.legend = tm_legend(
            title = "is_special_schome Distribution",
            orientation = "landscape", 
            position = c("BOTTOM", "LEFT"),
            item.height = 0.5, item.width = 2.1
        )
    ) + 
    tm_shape(is_special_schome_sdd_line, name = "SDD") + tm_lines(lwd = 2, lty = 1) + 
    tm_shape(is_special_schome_sde_line, name = "SDE") + tm_lines(lwd = 2, lty = 3) +
    tm_shape(is_special_schome_sde_weighted_line, name = "weighted SDE") + 
    tm_lines(col = "red", lwd = 2, lty = 3) +
    tm_shape(is_special_schome_mean_median, name = "Summary measures") + tm_dots(
        size = 0.5, shape = "id", fill = "white", 
        shape.legend = tm_legend("Summary measures")
    ) + 
    tm_add_legend(type = "lines", labels = "SDD", lty = 1) +
    tm_add_legend(type = "lines", labels = "SDE", lty = 3) +
    tm_add_legend(type = "lines", labels = "weighted SDE", lty = 3, col = "red") +
    tm_title("is_special_schome Distribution across SG Planning Areas")

print(is_special_schome_map)
```
...
pri_bc <- pri_cca_prevalence %>%
  mutate(
    sports_cat = cut(avg_cca_ratio, 
                     ...
                     is_special_schlude.lowest = TRUE),
...
census_merge_data <- census_merge %>% 
  st_drop_geometry() %>% 
  select(PLN_AREA_N, is_special_schome, Education) 
...
rmat <- cor(obesity_model_pri, use = "complete.obs")
...
rmat <- cor(obesity_model_sec, use = "complete.obs")
corrplot.mixed(rmat, upper = "pie", number.cex=0.75, tl.cex = 0.75,
               upper.col = COL2("PiYG"), lower.col = COL2("PiYG"))
...
Further, there is a negative linear relationship between overweight prevalence (PREVALENCE) and the

-   HMSP score (HMSP_Score),
-   Sports CCAs per 100 students (sports_cca_per_100),
-   Gross Monthly is_special_schome from Work by Planning Area (is_special_schome),
...
For the primary school variables, the intercept is the most statistically significant with p-value close to 0. Other statistically significant variables is_special_schlude is_coed_sch (p-value <0.01), is_popular_sch (p-value<0.05), while moderately significant variables are is_govt_sch and num_food_outlets (p-value<0.1)
For the secondary school variables, is_special_sch is the most statistically significant with p-value close to 0. Other statistically significant variables is_special_schlude the intercept (p-value<0.05) and is_popular_sch (p-value<0.1). 
...
This is corroborated by the variable analysis where is_coed_sch, is_popular_sch and is_govt_sch are recognised as statistically significant variables. Interestingly, the variable for Education is also is_special_schluded in the best linear model despite not being statistically significant in the earlier analysis. 
...
#Generating Best Fit OLS Model and Doing Usual Checks (Sec)

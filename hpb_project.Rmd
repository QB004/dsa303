...
summary(lm.all_sec)
```
For the primary school variables, the intercept is the most statistically significant with p-value close to 0. Other statistcally significant variables include is_coed_sch (p-value <0.01), is_popular_sch (p-value<0.05), while moderately significant variables are is_govt_sch and num_food_outlets (p-value<0.1)
...
This is corroborated by the variable analysis where is_coed_sch, is_popular_sch and is_govt_sch are recognised as statistically significant variables. Interesting, the variable for Education is also included in the best linear model despite not being statistically significant in the earlier analysis. 

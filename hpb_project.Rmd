...
# URA regions map
ura_regions <- sg_ura_boundaries
ura_region_map <- tm_shape(ura_regions) + tm_polygons(
  col = "grey", fill = "REGION_N", fill_alpha = 0.25, 
  fill.legend = tm_legend(title = "URA regions"))

# URA planning areas map
ura_planning_areas <- sg_ura_boundaries
ura_planning_map <- tm_shape(ura_planning_areas) + tm_polygons(
  col = "grey", fill = "PLN_AREA_N", fill_alpha = 0.25, 
  fill.legend = tm_legend_hide())

# URA subzones map
ura_subzones <- sg_ura_boundaries
ura_subzone_map <- tm_shape(ura_subzones) + tm_polygons(
...
moe_zone_map <- tm_shape(moe_zones) + tm_polygons(
...
moe_clusters <- obesity_data %>% group_by(CLUSTER) %>% 
  summarise(geometry = st_union(geometry)) %>% st_convex_hull()
moe_cluster_map <- tm_shape(moe_clusters) + tm_polygons(
  col = "grey", fill = "CLUSTER", fill_alpha = 0.25, 
  fill.legend = tm_legend(title = "MOE School Clusters"))
```
...
ura_region_map + ura_planning_map + ura_subzone_map + 
...
pri_sch_sde_weghted_line <- pri_sch_sde_weighted$LOCATIONS %>% 
  st_as_sf(coords = c("x", "y"), crs = sg_crs) %>% 
  st_combine() %>% 
  st_cast("LINESTRING")
...
pri_sch_map <- ura_region_map +
  ...
  tm_shape(pri_sch_sdd_line) + tm_lines(lwd = 2, lty = 1) + 
  tm_shape(pri_sch_sde_line) + tm_lines(lwd = 2, lty = 3) +
  tm_shape(pri_sch_sde_weghted_line) + tm_lines(col = "red", lwd = 2, lty = 3) +
  tm_shape(pri_sch_mean_median) + tm_dots(
...
sec_sch_sde_weghted_line <- sec_sch_sde_weighted$LOCATIONS %>% 
  st_as_sf(coords = c("x", "y"), crs = sg_crs) %>% 
  st_combine() %>% 
  st_cast("LINESTRING")
...
sec_sch_map <- ura_region_map +
  ...
  tm_shape(sec_sch_sdd_line) + tm_lines(lwd = 2, lty = 1) + 
  tm_shape(sec_sch_sde_line) + tm_lines(lwd = 2, lty = 3) +
  tm_shape(sec_sch_sde_weghted_line) + tm_lines(col = "red", lwd = 2, lty = 3) +
  tm_shape(sec_sch_mean_median) + tm_dots(
    size = 0.5, shape = "id", fill = "white", 
    shape.legend = tm_legend("Summary measures")
    ) + 
  tm_add_legend(type = "lines", labels = "SDD", lty = 1) +
...
obesity_joined_sports_data <- 
  obesity_filtered %>% left_join(school_ccas, by = join_by(SCHOOL == school_name))
...
sports_ccas_map <- tm_shape(obesity_joined_sports_data) +
  tm_symbols(
    size = 0.5,
    shape = "LEVEL",
    shape.legend = tm_legend(
    ...
    fill = "percentage_physical_sports", 
    fill.scale = tm_scale_continuous(
      values = "brewer.rd_yl_gn"), 
    fill.legend = tm_legend(
      title = "Physical sports participation by school",
      orientation = "landscape", 
      position = c("BOTTOM", "LEFT"),
      item.height = 0.5, item.width = 2.1
      )
...
school_level_sports_map <- tm_shape(obesity_joined_sports_data) +
  tm_symbols(
    size = 0.5,
    shape = "LEVEL",
    shape.legend = tm_legend(
...
tm_shape(obesity_joined_sports_data) +
  tm_bubbles(
    shape = "LEVEL",                
    size = "PREVALENCE",   #(size.scale = "PREVALENCE" - size too big)    
    col = "percentage_physical_sports",    
    palette = "brewer.rd_yl_gn",               
    border.col = "black",               
    title.size = "Obesity Prevalence (%)",
    title = "Physical Sports Participation (%)"
  ) +
  tm_layout(
    title = "Obesity Prevalence vs Physical Sports Participation",
    legend.outside = TRUE 

...
```{r}
library(tidyverse); library(sf); library(tmap); library(aspace); library(spdep); library(spatialreg); library(corrplot); library(readxl); library(gridExtra); library(imputeTS); library(seasonal); library(fpp2); library(urca); library(vars); library(trend)
```
...
obesity_pri_sch %>% filter(LMORANZ >= 3)

# Identified clusters in Secondary schools
obesity_sec_sch %>% filter(LMORANZ >= 3)
```
...
pri_sar <- spautolm(formula = PREVALENCE ~ 1, data = obesity_model_analysis_pri, 
                    listw = del_lw_pri)
summary(pri_sar) # Lambda: 0.51079, p-value: 3.1092e-08 

pri_slm <- lagsarlm(
  formula = PREVALENCE ~ Education + is_popular_sch + is_govt_sch + is_coed_sch, 
  data = obesity_model_analysis_pri, listw = del_lw_pri)
summary(pri_slm) # Rho: 0.22439, p-value: 0.014291

pri_sem <- errorsarlm(
  formula = PREVALENCE ~ Education + is_popular_sch + is_govt_sch + is_coed_sch,
  data = obesity_model_analysis_pri, listw = del_lw_pri)
summary(pri_sem) # Lambda: 0.23191, p-value: 0.05814

pri_sarar <- sacsarlm(
  formula = PREVALENCE ~ Education + is_popular_sch + is_govt_sch + is_coed_sch,
  data = obesity_model_analysis_pri, listw = del_lw_pri)
summary(pri_sarar) 
# Rho: 0.36004, p-value: 0.0073521; Lambda: -0.24314, p-value: 0.25526
```
...
lm.LMtests(ols_pri, listw = del_lw_pri, test = "all") 
# RSerr = 3.5693, df = 1, p-value = 0.05886
# RSlag = 6.9336, df = 1, p-value = 0.008459
# adjRSerr = 0.15306, df = 1, p-value = 0.6956
# adjRSlag = 3.5174, df = 1, p-value = 0.06073
# SARMA = 7.0867, df = 2, p-value = 0.02892

# Likelihood Ratio tests

LR.Sarlm(pri_sem, pri_sarar) # p-value = 0.07339
LR.Sarlm(pri_slm, pri_sarar) # p-value = 0.3731
LR.Sarlm(pri_slm, pri_sar) # p-value < 2.2e-16
LR.Sarlm(pri_sem, pri_sar) # p-value = 4.441e-16

LR1.Sarlm(pri_slm) # p-value = 0.01429
LR1.Sarlm(pri_sem) # p-value = 0.05814

# Comparing AIC 
AIC(pri_slm) # -923.8595
AIC(pri_sem) # -921.4472
AIC(ols_pri) # -919.8576

# Comparing BIC
BIC(pri_slm) # -901.47
BIC(pri_sem) # -899.0578
...
sec_sar <- spautolm(formula = PREVALENCE ~ 1, data = obesity_model_analysis_sec, 
                    listw = del_lw_sec)
summary(sec_sar) # Lambda: 0.34546, p-value: 0.0031651 

sec_slm <- lagsarlm(
  formula = PREVALENCE ~ Education + is_popular_sch + is_govt_sch + is_coed_sch, 
  data = obesity_model_analysis_sec, listw = del_lw_sec)
summary(sec_slm) # Rho: 0.1198, p-value: 0.26376

sec_sem <- errorsarlm(
  formula = PREVALENCE ~ Education + is_popular_sch + is_govt_sch + is_coed_sch,
  data = obesity_model_analysis_sec, listw = del_lw_sec)
summary(sec_sem) # Lambda: 0.11874, p-value: 0.3936

sec_sarar <- sacsarlm(
  formula = PREVALENCE ~ Education + is_popular_sch + is_govt_sch + is_coed_sch,
  data = obesity_model_analysis_sec, listw = del_lw_sec)
summary(sec_sarar) 
# Rho: 0.16199, p-value: 0.3618; Lambda: -0.068583, p-value: 0.7756
```
...
lm.LMtests(ols_sec, listw = del_lw_sec, test = "all") 
# RSerr = 1.392, df = 1, p-value = 0.2381
# RSlag = 2.1613, df = 1, p-value = 0.1415
# adjRSerr = 0.0060261, df = 1, p-value = 0.9381
# adjRSlag = 0.77534, df = 1, p-value = 0.3786
# SARMA = 2.1674, df = 2, p-value = 0.3383

# Likelihood Ratio tests

LR.Sarlm(sec_sem, sec_sarar) # p-value = 0.4452
LR.Sarlm(sec_slm, sec_sarar) # p-value = 0.8036
LR.Sarlm(sec_slm, sec_sar) # p-value = 3.331e-15
LR.Sarlm(sec_sem, sec_sar) # p-value = 4.33e-15

LR1.Sarlm(sec_slm) # p-value = 0.2638
LR1.Sarlm(sec_sem) # p-value = 0.3936

# Comparing AIC 
AIC(sec_slm) # -598.4831
AIC(sec_sem) # -597.962
AIC(ols_sec) # -601.9956

# Comparing BIC
BIC(sec_slm) # -577.6944
BIC(sec_sem) # -577.1733
...
oso_hist_geom = oso_hist_geom_full %>%
  dplyr::select(c("SCHOOL", "geometry", "z_change"))

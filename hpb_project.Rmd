...
# Assuming you have two data frames or sf objects:
# schools: contains locations of schools
# facilities: contains locations of sports facilities


# Step 2: Calculate distances between all schools and facilities
distances <- st_distance(schools_sf, facilities_sf)

# Step 3: For each school, find the k nearest facilities
k <- 3  # Change this to your desired k value
nearest_facilities <- apply(distances, 1, function(dists) {
  order(dists)[1:k]
})

# Create a data frame with the results
results <- data.frame(
  school_id = schools_sf$id,  # Replace with your actual ID column
  stringsAsFactors = FALSE
)

# Add the nearest facilities for each school
for (i in 1:k) {
  facility_col <- paste0("nearest_facility_", i)
  distance_col <- paste0("distance_", i)
  
  results[[facility_col]] <- sapply(1:nrow(schools_sf), function(j) {
    facilities_sf$id[nearest_facilities[i, j]]  # Replace with your actual ID column
  })
  
  results[[distance_col]] <- sapply(1:nrow(schools_sf), function(j) {
    distances[j, nearest_facilities[i, j]]
  })
}
```

...
p-value for Global Moran's I test (Primary school): 0.006533
p-value for Global Geary's C test (Primary school): 0.005138
p-value for Global Getis-Ord's G test (Primary school): 0.8745
...
p-value for Global Moran's I test (Primary school): 4.515e-10
p-value for Global Geary's C test (Primary school): 9.073e-10
p-value for Global Getis-Ord's G test (Primary school): 0.05955
...
p-value for Global Moran's I test (Primary school): 2.581e-06
p-value for Global Geary's C test (Primary school): 2.736e-06
p-value for Global Getis-Ord's G test (Primary school): 0.05455
...
p-value for Global Moran's I test (Primary school): 0.008449
p-value for Global Geary's C test (Primary school): 0.04445
p-value for Global Getis-Ord's G test (Primary school): 0.1256
...
Average number of links (Primary school): 2
p-value for Global Moran's I test (Primary school): 6.805e-05
p-value for Global Geary's C test (Primary school): 0.000133
p-value for Global Getis-Ord's G test (Primary school): 0.01034
...
Average number of links (Primary school): 3
p-value for Global Moran's I test (Primary school): 2.337e-06
p-value for Global Geary's C test (Primary school): 1.103e-06
p-value for Global Getis-Ord's G test (Primary school): 0.005879
...
st_crs(seven_eleven)$epsg #Check CRS
  ...
  #Malls
malls_csv = read.csv("shopping_mall_coordinates.csv")
malls_full = st_as_sf(malls_csv, coords = c("LONGITUDE", "LATITUDE"), crs = 4326) %>%
  st_transform(crs = sg_crs)
st_crs(malls_full)
...
mall_sf = mutate(malls_full, outlet = "shopping mall")[-1]
food = rbind(seven_eleven_sf, macs_sf, hawker_sf, mall_sf)
food
...
nearest_outlet_distance = find_nearest_food_options(school_sf, food)
...
library(spatstat)   # For KDE calculation
library(raster)     # For raster conversion


# Step 2: Ensure all data have the same CRS
schools <- st_transform(schools, st_crs(ura_planning_areas))
sports_facilities <- st_transform(sports_facilities, st_crs(ura_planning_areas))

# Step 3: Calculate KDE for each planning area
answers <- list()
kde_rasters <- list()

# Get the name of the column that contains the planning area names
# Adjust this to match your actual column name
PLN_AREA_N <- "name"  # Change this to your column name like "PA_NAME" or similar

for(i in 1:nrow(ura_planning_areas)) {
  # Extract current planning area
  current_area <- ura_planning_areas[i,]
  area_name <- current_area[[PLN_AREA_N]]
  
  # Filter schools and facilities within this planning area
  area_schools <- schools[st_within(schools, current_area) %>% lengths > 0,]
  area_facilities <- sports_facilities[st_within(sports_facilities, current_area) %>% lengths > 0,]
  
  # Skip areas with too few facilities for meaningful KDE
  if(nrow(area_facilities) < 3) {
    cat("Skipping", area_name, "- insufficient facilities (", nrow(area_facilities), ")\n")
    next
  }
  
  # Create a window for KDE based on planning area boundary
  area_window <- as.owin(current_area)
  
  # Convert facilities to point pattern
  facilities_coords <- st_coordinates(area_facilities)
  facilities_ppp <- ppp(
    x = facilities_coords[,1], 
    y = facilities_coords[,2], 
    window = area_window
  )
  
  # Calculate KDE with appropriate bandwidth
  # Adjust sigma based on your data's units and density
  kde <- density(facilities_ppp, sigma = 1000, edge = TRUE)
  
  # Convert to raster for mapping
  kde_raster <- raster(kde)
  crs(kde_raster) <- st_crs(ura_planning_areas)$proj4string
  
  # Store raster
  kde_rasters[[area_name]] <- kde_raster
  
  # Get density values at school locations if there are schools in this area
  if(nrow(area_schools) > 0) {
    schools_coords <- st_coordinates(area_schools)
    density_at_schools <- extract(kde_raster, schools_coords)
    
    # Add to schools data
    area_schools$kde_value <- density_at_schools
    
    # Store results
    answers[[area_name]] <- area_schools
  }
}

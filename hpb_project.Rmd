...
cca_planning_avg_map <- ura_planning_areas_map + cca_planning_avg %>% 
  tm_shape(name = "Sport CCA Ratio per 100 Students") +
  tm_polygons(
    fill = "RATIO_AGG",
    fill.scale = tm_scale(
      #breaks = c(0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30),
...
dual_use_facilities_shp <- 
  st_read("DualUseSchoolsSportsFacilitiesSHP/DUS_School_Sports_Facilities.shp") %>% 
  st_make_valid() %>%
  st_as_sf() %>%
  st_transform(crs = sg_crs) 

dus_facility_map <- ura_planning_areas_map + tm_shape(dual_use_facilities_shp) +
    tm_dots(size = 0.3,
          fill = "navy",
...
pri_dus_map <- ura_planning_areas_map + 
  ...
  tm_shape(dual_use_facilities_shp) +
  tm_dots(
    size = 0.5,
    fill = "blue"
  ) +

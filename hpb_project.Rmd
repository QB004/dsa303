...
```{r}
start.time = Sys.time()
pkgs = c("dplyr", "ggplot2", "tidyr", "readr", "crayon", "tibble", "knitr", "gt", "broom", "DT", "tidyverse", "sf", "tmap", "aspace", "spdep", "corrplot", "readxl", "spatialreg")
...
a <- tm_shape(sg_ura_boundaries) + tm_fill() + tm_borders() + tm_shape(hmsp_data_sf) + 
  tm_symbols(size = 0.4, fill = "HMSP_Tier", 
             fill.scale = tm_scale(values = "brewer.set1"),
             fill.legend = tm_legend(
               title = "HMSP Tiers", bg.color = "white", 
               position = tm_pos_in(pos.h="right", pos.v="bottom")
               ))

b <- tm_shape(sg_ura_boundaries) + tm_fill() + tm_borders() + tm_shape(hmsp_data_sf) + 
  tm_symbols(size = 0.4, fill = "ZONE", 
             fill.scale = tm_scale(values = "brewer.set1"),
             fill.legend = tm_legend(
               title = "School zones", bg.color = "white", 
               position = tm_pos_in(pos.h = "right", pos.v = "bottom")
               ))
...
tm_shape(sg_ura_boundaries) + tm_fill() + tm_borders() +
  ...
  tm_symbols(size = 0.3, fill = "CLUSTER", 
             fill.scale = tm_scale(values = "brewer.set1"), 
             fill.legend = tm_legend(
               title = "School clusters", bg.color = "white", 
               position = tm_pos_out(cell.h = "right"))) +
  tm_facets_wrap(by = "ZONE", nrow = 2)
```

We first visualised the spread of HMSP Tiers, school zones and school clusters in Singapore to gain an initial intuition of the dataset that HPB provided.
...
Schools with specialised programmes tend to have lower rates of obesity.
...
The map above is an overview of overweight prevalence in Singapore schools. You may use the different layers provided in the map to view the individual school data against URA regions, planning areas and subzones, or MOE zones and clusters. Note that MOE zones and clusters have overlapping areas. The lighter yellow symbols represent schools with relatively lower obesity levels, while darker red symbols represent schools with relatively higher obesity levels. The circle symbols represent primary schools, while the square symbols represent secondary schools.
...
pri_sch_map_summ <- pri_sch_map +
  ...
  tm_shape(pri_sch_sde_line, name = "SDE") + tm_lines(col = "blue", lwd = 2, lty = 3) +
  tm_shape(pri_sch_sde_weighted_line, name = "weighted SDE") + 
  tm_lines(col = "red", lwd = 2, lty = 3) +
  tm_shape(pri_sch_mean_median, name = "Spatial central tendency") + 
  tm_dots(size = 0.5, shape = "id", fill = "white", shape.legend = tm_legend("Spatial central tendency")) + 
  tm_add_legend(labels=c("weighted SDE", "SDE", "SDD"), type="lines", 
                col=c("red", "blue", "black"), title="Spatial variation", lty=c(3,3,1), lwd=2) +
...
sec_sch_map_summ <- sec_sch_map +
  ...
  tm_shape(sec_sch_sde_line, name = "SDE") + tm_lines(col = "blue", lwd = 2, lty = 3) +
  tm_shape(sec_sch_sde_weighted_line, name = "weighted SDE") + 
  tm_lines(col = "red", lwd = 2, lty = 3) +
  tm_shape(sec_sch_mean_median, name = "Spatial central tendency") + tm_dots(
    size = 0.5, shape = "id", fill = "white", 
    shape.legend = tm_legend("Spatial central tendency")
    ) + 
  tm_add_legend(labels=c("weighted SDE", "SDE", "SDD"), type="lines", 
                col=c("red", "blue", "black"), title="Spatial variation", lty=c(3,3,1), lwd=2) +
...
moe_zones_map + obesity_moe_zones %>% 
  tm_shape(name = "Overweight prevalence") +
  tm_polygons(
    fill = "PREVALENCE_AGGREGATE",
    fill_alpha = 0.95,
...
moran.plot(obesity_ura_planning_areas_pri$PREVALENCE_AGGREGATE, ura_planning_areas_lw_pri,
           ...
           xlab="Overweight prevalence", ylab="Spatially lagged Overweight prevalence")
...
There are 323 URA subzones in Singapore, of which 115 contain primary schools, and 100 contain secondary schools. As there were are many non-contiguous subzones, we do not run our spatial autocorrelation statistics here.
...
```{r}
# Summary of all spatial autocorrelation test p-values
nb_pval_pri %>% gt() %>%
  tab_header(
...
Using Geary’s C test, we can reject the null hypothesis at the 1% significance level for most spatial weight matrices, further supporting the presence of spatial dependence and reinforcing the finding of clustering, with the exception of social neighbours (MOE clusters) and k = 1 nearest neighbours. 

Getis-Ord’s G test results are less consistent. We do not reject the null hypothesis at the 1% significance level for all spatial weight matrices. We reject the null hypothesis at the 10% significance level for all spatial weight matrices except Planning Area. Some configurations (Delaunay triangulation, Sphere of Influence, and k = 1, 2, 3 nearest neighbours) continue to suggest moderately significant (p-values < 0.10) clustering of obesity in Primary schools.
...
The insignificant results for social neighbours (for both primary and secondary schools) indicates that there is insufficient evidence to suggest that "social distance" might lead to spatial clustering of obesity in schools. From a policy perspective, administrators of MOE clusters do not influence obesity in schools, through MOE cluster specific policies or programmes for example. This could be due to MOE's macro approach to administrating policies across all schools in Singapore. Evidently, MOE works with HPB to implement the HMSP nationwide.

The insignificant results for contiguous neighbours (for only secondary schools) indicates that there is insufficient evidence to suggest that planning area specific factors might lead to spatial clustering of obesity in schools. Schools in different planning areas are unlikely to affect each other due to the distance apart and different residential, commercial, business and recreational areas that students may go to (shopping malls, parks, etc.).
...
jc_pri <- joincount.multi(obesity_pri_sch$HIGH_PREVALENCE, soi_lw_B_pri)
jc_pri %>% as.data.frame() %>% gt(rownames_to_stub = TRUE) %>% 
  tab_header(
    title = "Join count analysis (SOI)",
    subtitle = "Primary schools"
  ) %>%
  fmt_number(
    columns = c("Expected", "Variance", "z-value"),
    decimals = 4
  )
...
jc_sec <- joincount.multi(obesity_sec_sch$HIGH_PREVALENCE, soi_lw_B_sec)
jc_sec %>% as.data.frame() %>% gt(rownames_to_stub = TRUE) %>% 
  tab_header(
    title = "Join count analysis (SOI)",
    subtitle = "Secondary schools"
  ) %>%
...
#Hawker Centres
hawker_full = suppressMessages(st_read("DataFiles/NEAMarketandFoodCentre.geojson")) %>%
  st_transform(crs = sg_crs)
hawker = hawker_full["geometry"]

#Fast Food Outlets 
  #Mcdonald's
macs_full = suppressMessages(st_read("DataFiles/mcdonalds_singapore.geojson")) %>%
  st_transform(crs = sg_crs)
macs = macs_full[,"geometry"] 

#7-11 Outlets
seven_eleven_full = suppressMessages(st_read("DataFiles/711_singapore.geojson")) %>%
  st_transform(crs = sg_crs)
...
pri_bivariate_food_map <-ura_planning_areas_map +
  ...
  tm_polygons(fill = "bivariate_class", palette = bivariate_colors, fill.legend = tm_legend("Bivariate Categories (Food - Obesity)")) +
...
Based on the sign of the correlation coefficient values, we can observe a positive linear relationship between overweight prevalence (PREVALENCE) and the
...
Next, we consider the 4 spatial models to see whether spatial relationships should be considered for our model: 

-   Pure Spatial AutoRegressive Model (SAR)
...
nb_lm_pri <- nb_lm_sec <- 
  data.frame("H0" = c(), "H1" = c(), 
             "P.value" = c(),"Test.Result" = c())
...
nb_lm_pri <- rbind(nb_lm_pri, data.frame(
  "H0" = c("OLS","OLS"), "H1" =c("SEM","SLM"), "P.value" = c(0.5148,0.1187),"Test.Result" = c("Do not reject H0","Do not reject H0")))
...
nb_lr_pri <- rbind(nb_lr_pri, data.frame(
  "H0" = c("SEM","SLM","SAR","SAR","OLS","OLS"), "H1" =c("SARAR","SARAR","SEM","SLM","SEM","SLM"), "P.value" = c(0.5148,0.275,"< 2.2e-16","< 2.2e-16",0.1725,0.0497),"Test.Result" = c("Do not reject H0","Do not reject H0","Reject H0","Reject H0","Do not reject H0","Reject H0")))

# Comparing AIC 
nb_abic_pri <- nb_abic_sec <- 
  data.frame("Criterion" = c(), "OLS" = c(), 
             "SLM" = c(),"SEM" = c())
AIC(pri_slm) # -921.7105
AIC(pri_sem) # -919.7185
AIC(ols_pri) # -919.8576
nb_abic_pri <- rbind(nb_abic_pri, data.frame(
  "Criterion" = "AIC", "OLS" = AIC(ols_pri), "SLM" = AIC(pri_slm),"SEM" = AIC(pri_sem)))
...
nb_abic_pri <- rbind(nb_abic_pri, data.frame(
  "Criterion" = "BIC", "OLS" = BIC(ols_pri), "SLM" = BIC(pri_slm),"SEM" = BIC(pri_sem)))
```
...
nb_lr_pri %>% gt() %>%
    ...
    fmt_number(
        columns = c(H0,H1,P.value,Test.Result),
        decimals = 4
    )
nb_lm_pri %>% gt() %>%
    tab_header(
        title = "Lagrange Multiplier Test Results",
        subtitle = "Primary schools"
    ) %>%
    fmt_number(
        columns = c(H0,H1,P.value,Test.Result),
...
nb_abic_pri %>% gt() %>%
    tab_header(
        title = "Model AIC & BIC Values",
        subtitle = "Primary schools"
    ) %>%
    fmt_number(
        columns = c(Criterion,OLS,SLM,SEM),
        decimals = 4
    )
```

Using the LR tests, we see the tests with SARAR as alternative hypothesis that the large p-values for both tests mean SLM and SEM are more suitable than SARAR. The extremely small p-value for the test between SAR and SLM indicates the alternative SLM is more suitable than SAR. For the tests with OLS as the null, we see that when SLM is the alternative the p-value is small (0.0497) which is sufficient evidence at 5% significance level that the alternative SLM is more suitable. On the other hand, when SEM is the alternative, the p-value is much larger (0.1725), so there is insufficient evidence at 5% significance level to reject OLS. This is further supported by the LM test results, where adjRSlag has a much smaller p-value of 0.1187 as compared to adjRSerr p-value of 0.5148. This suggests the SLM is more suitable than SEM, though the significance of SLM relative to OLS may be difficult to determine at 5% significance level from this test alone. Finally, considering the AIC and BIC values, we see pri_slm has a marginally smaller AIC value than SEM and OLS. 

This brings us to our conclusion that SLM is the most suitable spatial model for modelling obesity prevalence in primary schools. This is supported by our prior spatial analysis, where we recognised both high-obesity & low-obesity clustering being observed for several primary schools. Logically, this also makes sense since the student residences for primary schools tend to be more concentrated within the same neighbourhood due to the nature of primary school slot allocation in Singapore.
...
nb_lm_sec <- rbind(nb_lm_sec, data.frame(
  "H0" = c("OLS","OLS"), "H1" =c("SEM","SLM"), "P.value" = c(0.4382,0.1583),"Test.Result" = c("Do not reject H0","Do not reject H0")))
...
nb_lr_sec <- rbind(nb_lr_sec, data.frame(
  "H0" = c("SEM","SLM","SAR","SAR","OLS","OLS"), "H1" =c("SARAR","SARAR","SEM","SLM","SEM","SLM"), "P.value" = c(0.6651,0.6771,"2.331e-15","2.331e-15",0.08286,0.08358),"Test.Result" = c("Do not reject H0","Do not reject H0","Reject H0","Reject H0","Do not reject H0","Do not reject H0")))

# Comparing AIC 
AIC(sec_slm) # -600.2422
AIC(sec_sem) # -600.2281
AIC(ols_sec) # -601.9956
nb_abic_sec <- rbind(nb_abic_sec, data.frame(
  "Criterion" = "AIC", "OLS" = AIC(ols_sec), "SLM" = AIC(sec_slm),"SEM" = AIC(sec_sem)))
...
nb_abic_sec <- rbind(nb_abic_sec, data.frame(
  "Criterion" = "BIC", "OLS" = BIC(ols_sec), "SLM" = BIC(sec_slm),"SEM" = BIC(sec_sem)))
```
...
Using the LR tests, we see the tests with SARAR as alternative hypothesis that the large p-values for both tests mean SLM and SEM are more suitable than SARAR. The extremely small p-value for the test between SAR and SLM indicates the alternative SLM is more suitable than SAR. For the tests with OLS as the null, we see in both tests where SLM and SEM are alternatives that the p-value is much larger than 0.05, showing there is insufficient evidence at 5% significance level to reject the null OLS. This is further supported by the LM test results, where adjRSlag and adjRSerr p-values are very large and greater than 0.05. This suggests the OLS may be a more suitable model than the spatial models SLM and SEM. Finally, considering the AIC and BIC values, we see ols_sec has smaller AIC and BIC values than both SEM and OLS. 

This brings us to our conclusion that OLS is the most suitable for modelling obesity prevalence in primary schools, more so than any type of spatial model. We can attribute this to secondary school students' residences being more dispersed, especially in the case of popular schools such as RI and HCI. This means that localised clustering of obesity prevalence becomes less relevant for secondary schools, which favours the OLS model over spatial models. 
...
oso_increase_gt <- oso_hist_geom %>% 
  st_drop_geometry() %>% 
  filter(z_change >= 1) %>% 
  arrange(desc(z_change)) %>%
  gt() %>%
  tab_header(
    title = "Schools with ≥ 1 SD Increase in OSO Levels"
  )

oso_increase_gt
  
# Schools which saw a 1 standard deviation decrease in OSO levels
oso_decrease_gt <- oso_hist_geom %>% 
  st_drop_geometry() %>% 
  filter(z_change <= -1) %>% 
  arrange(z_change) %>%  # ascending since they're negative
  gt() %>%
  tab_header(
    title = "Schools with ≥ 1 SD Decrease in OSO Levels"
  )
oso_decrease_gt

```
A histogram plot of the data shows that the data is normally distributed.
For the first model, we interpolated the data to obtain the missing years between 2016 to 2023. We used a smoothing spline to do this, and subsequently calculated the mean Z-score before 2019 and after 2019. 2019 was used as the baseline for the midpoint in the data. 

We then obtained schools with a more than 1 Standard Deviation increase and decrease in OSO levels over time. 
...
moran_result <- moran.test(oso_hist_geom$z_change, soi_lw_pri_z)
geary_result <- geary.test(oso_hist_geom$z_change, soi_lw_pri_z)
...
geary_df <- data.frame(
  Test = "Geary's C",
  Statistic = geary_result$statistic,
  P_Value = geary_result$p.value
)

# Combine and format
bind_rows(moran_df, geary_df) %>%
  gt() %>%
  tab_header(
    title = "Spatial Autocorrelation Tests",
    subtitle = "Z-Change in OSO Levels (Primary Schools)"
  ) %>%
  fmt_number(
...
p-value for Global Moran's I test (Primary school): 0.5712\ 

p-value for Global Geary's C test (Primary school): 0.3879
...
### Join count considering only extremes
...
jc_decrease <- joincount.multi(oso_hist_geom3$z_change_high_decrease, soi_lw_B_pri_z_decrease)
jc_increase <- joincount.multi(oso_hist_geom1$z_change_high_increase, soi_lw_B_pri_z_increase)

jc_decrease_df <- as.data.frame(jc_decrease)
jc_increase_df <- as.data.frame(jc_increase)

jc_decrease_df <- jc_decrease_df %>% mutate(across(where(is.numeric), ~round(., 4)))
jc_increase_df <- jc_increase_df %>% mutate(across(where(is.numeric), ~round(., 4)))

gt_decrease <- jc_decrease_df %>%
  gt(rownames_to_stub = TRUE) %>%
  tab_header(
    title = "Join Count Test – High Decrease",
    subtitle = "Primary Schools"
  )

gt_increase <- jc_increase_df %>%
  gt(rownames_to_stub = TRUE) %>%
  tab_header(
    title = "Join Count Test – High Increase",
    subtitle = "Primary Schools"
  )
...
before_summary = summary(oso_covid_geom$before_covid_change)
after_summary = summary(oso_covid_geom$after_covid_change)

before_summary_df <- data.frame(
  Statistic = names(before_summary),
  Value = as.numeric(before_summary)
)

before_summary_df %>%
  gt() %>%
  tab_header(
    title = "Summary of Obesity Changes Before COVID",
    subtitle = "Primary schools"
  ) %>%
  fmt_number(
    columns = Value,
...
after_summary_df <- data.frame(
  Statistic = names(after_summary),
  Value = as.numeric(after_summary)
)

after_summary_df %>%
  gt() %>%
  tab_header(
    title = "Summary of Obesity Changes After COVID",
    subtitle = "Primary schools"
  ) %>%
  fmt_number(
    columns = Value,
...
From our spatial analysis, we found that there is directional bias for obesity in schools, with the North of Singapore having more schools with higher obesity levels. Our identification of spatial clusters further supported this finding, with evidence of 2 High-High obesity clusters of primary schools in the North. Low-Low obesity clusters were also found towards the Center and South-West of Singapore. Policy-makers can leverage on the spillover effects, between schools in these clusters, to implement obesity reduction or mitigation measures. One limitation of our autocorrelation analysis is that we did not have data to explain why these clusters occur. However, we suspect that social influences and peer pressure has a big part to play, as HPB had shared that student profiles in schools in the North are vastly different from other regions, with higher incidences of smoking and bullying for example. Common hangout spots (captured within the sphere of influence definition), like shopping malls or parks, may also play into the social influence and spatial dependence between schools in the North.

Including other explanatory variables for obesity (education, income, access to external food options, access to sports opportunities), we built linear regression models with the focus on explaining obesity in schools. We found the variable Education (parental education in cumulative years) to be common to both models, which marked it as a possible significant factor for future targeting. Following which, we compared the generated best linear models with spatial models for both primary & secondary schools. For primary schools, we found SLM to be the best model, corroborating our earlier analysis on the existence of High-High and Low-Low obesity spatial clusters of primary schools in Singapore. For secondary schools, we found OLS prevailed as the best model, which could be attributed to higher dispersion of secondary school student residences so spatial effects were far less significant. 

Finally, we explored a time series analysis of obesity prevalence over time. From our first model, we found no spatial autocorrelation of extreme changes in obesity over the long run. Therefore, there were no spatial spillover effects from schools with extreme changes in obesity. In our second model, we controlled for COVID effects. We were able to highlight schools that experienced increasing obesity levels after short-term disruptions caused by the COVID pandemic and Circuit Breaker that led to a period of home-based learning.

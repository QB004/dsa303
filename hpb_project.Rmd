...
## Spatial dependence of obesity in schools

### Visualisation of contiguous neighbours by region

There are too few contiguous neighbours by URA region to consider (only 5 URA regions). When analysing by URA subzone, there are too many non-contiguous subzones. We recommend to only analyse by URA planning area, which has some non-contiguous neighbours in the central regions. These non-contiguous neighbours by planning area are then manually edited (in the .gal file) to create neighbours with nearest planning areas to ensure contiguity for spatial statistical tests.
...
ura_regions_nb_PRI <- obesity_ura_regions_PRI %>% 
  poly2nb(queen = T)
ura_regions_nb_SEC <- obesity_ura_regions_SEC %>% 
  poly2nb(queen = T)
ura_regions_coords_PRI <-
  st_point_on_surface(st_geometry(obesity_ura_regions_PRI))
ura_regions_coords_SEC <-
  st_point_on_surface(st_geometry(obesity_ura_regions_SEC))
par(mfrow = c(2,1))
plot(st_geometry(ura_regions), border = "grey", axes = T, 
     main="Primary schools contiguous neighbours by region")
plot.nb(ura_regions_nb_PRI, ura_regions_coords_PRI, 
        pch = 19, cex = 0.6, add = T)
plot(st_geometry(ura_regions), border = "grey", axes = T, 
     main="Secondary schools contiguous neighbours by region")
plot.nb(ura_regions_nb_SEC, ura_regions_coords_SEC, 
        pch = 19, cex = 0.6, add = T)
par(mfrow = c(1,1))
...
ura_planning_areas_nb_PRI <- obesity_ura_planning_areas_PRI %>% 
  poly2nb(queen = T)
ura_planning_areas_nb_SEC <- obesity_ura_planning_areas_SEC %>% 
  poly2nb(queen = T)
ura_planning_areas_coords_PRI <-
  st_point_on_surface(st_geometry(obesity_ura_planning_areas_PRI))
ura_planning_areas_coords_SEC <-
  st_point_on_surface(st_geometry(obesity_ura_planning_areas_SEC))
par(mfrow = c(2,1))
plot(st_geometry(ura_planning_areas), border = "grey", axes = T, 
     main="Primary schools contiguous neighbours by planning area")
plot.nb(ura_planning_areas_nb_PRI, ura_planning_areas_coords_PRI, 
        pch = 19, cex = 0.6, add = T)
plot(st_geometry(ura_planning_areas), border = "grey", axes = T, 
     main="Secondary schools contiguous neighbours by planning area")
plot.nb(ura_planning_areas_nb_SEC, ura_planning_areas_coords_SEC, 
        pch = 19, cex = 0.6, add = T)
par(mfrow = c(1,1))

# Contiguous neighbours by URA subzones
obesity_ura_subzones_PRI <- obesity_ura_subzones %>% 
  filter(LEVEL == "PRIMARY")
obesity_ura_subzones_SEC <- obesity_ura_subzones %>% 
  filter(LEVEL == "SECONDARY")
ura_subzones_nb_PRI <- obesity_ura_subzones_PRI %>% 
  poly2nb(queen = T)
ura_subzones_nb_SEC <- obesity_ura_subzones_SEC %>% 
  poly2nb(queen = T)
ura_subzones_coords_PRI <-
  st_point_on_surface(st_geometry(obesity_ura_subzones_PRI))
ura_subzones_coords_SEC <-
  st_point_on_surface(st_geometry(obesity_ura_subzones_SEC))
par(mfrow = c(2,1))
plot(st_geometry(ura_subzones), border = "grey", axes = T, 
     main="Primary schools contiguous neighbours by subzone")
plot.nb(ura_subzones_nb_PRI, ura_subzones_coords_PRI, 
        pch = 19, cex = 0.6, add = T)
plot(st_geometry(ura_subzones), border = "grey", axes = T, 
     main="Secondary schools contiguous neighbours by subzone")
plot.nb(ura_subzones_nb_SEC, ura_subzones_coords_SEC, 
        pch = 19, cex = 0.6, add = T)
par(mfrow = c(1,1))
...
knn2_nb_PRI <- obesity_pri_sch %>% 
  knearneigh(k = 2) %>% 
  knn2nb(row.names = obesity_pri_sch$PT_ID) # nearest 2 neighbours
knn3_nb_PRI <- obesity_pri_sch %>% 
  knearneigh(k = 3) %>% 
  knn2nb(row.names = obesity_pri_sch$PT_ID) # nearest 3 neighbours
...
knn2_nb_SEC <- obesity_sec_sch %>% 
  knearneigh(k = 2) %>% 
  knn2nb(row.names = obesity_sec_sch$PT_ID) # nearest 2 neighbours
knn3_nb_SEC <- obesity_sec_sch %>% 
  knearneigh(k = 3) %>% 
  knn2nb(row.names = obesity_sec_sch$PT_ID) # nearest 3 neighbours

par(mfcol = c(3,2))
plot(st_geometry(ura_planning_areas), border="grey", axes=T,  main="k=1 (PRI)")
plot.nb(knn1_nb_PRI, st_geometry(obesity_pri_sch), pch=19, cex=0.6, add=T)
plot(st_geometry(ura_planning_areas), border="grey", axes=T,  main="k=2 (PRI)")
plot.nb(knn2_nb_PRI, st_geometry(obesity_pri_sch), pch=19, cex=0.6, add=T)
plot(st_geometry(ura_planning_areas), border="grey", axes=T,  main="k=3 (PRI)")
plot.nb(knn3_nb_PRI, st_geometry(obesity_pri_sch), pch=19, cex=0.6, add=T)
plot(st_geometry(ura_planning_areas), border="grey", axes=T,  main="k=1 (SEC)")
plot.nb(knn1_nb_SEC, st_geometry(obesity_sec_sch), pch=19, cex=0.6, add=T)
plot(st_geometry(ura_planning_areas), border="grey", axes=T,  main="k=2 (SEC)")
plot.nb(knn2_nb_SEC, st_geometry(obesity_sec_sch), pch=19, cex=0.6, add=T)
plot(st_geometry(ura_planning_areas), border="grey", axes=T,  main="k=3 (SEC)")
plot.nb(knn3_nb_SEC, st_geometry(obesity_sec_sch), pch=19, cex=0.6, add=T)
```
...
obesity_ura_regions$HIGH_PREVALENCE <- 
...
joincount.multi(obesity_ura_regions$HIGH_PREVALENCE, listw = ura_regions_lw_B) # too few cells
...
obesity_ura_planning_areas$HIGH_PREVALENCE <- 
...
joincount.multi(obesity_ura_planning_areas$HIGH_PREVALENCE, listw = ura_planning_areas_lw_B) 
...
obesity_ura_subzones$HIGH_PREVALENCE <- 
...
joincount.multi(obesity_ura_subzones$HIGH_PREVALENCE, listw = ura_subzones_lw_B) 
```
...
#   tm_title("Local Moran's I Z-Statistic (URA planning_areas)")
...
cca_ura_planning <- st_join(ura_planning_areas, obesity_joined_school_sports_data) %>% 
  group_by(PLN_AREA_N, LEVEL) %>% 
  summarise(
...
cca_planning_map <- ura_planning_areas_map + cca_ura_planning %>% 
  ...
  tm_title("Average Sports CCA Ratio per 100 Students by URA Planning Area") +
  tm_facets_wrap(by = "LEVEL")


print(cca_planning_map)
...
head(sports_facilities_sf)

test_map <- ura_planning_areas_map + tm_shape(sports_facilities_sf) +
...
test_two <- ura_planning_areas_map + tm_shape(fields_shp) +
    tm_dots(size = 0.3,
          fill = "slateblue2",
          fill.legend = tm_legend(title = "Sports Fields"),
          fill_alpha = 0.5)
...
# Loading School Sports facilities (dual use) data
dus_facilities_shp <- 
...
test_three <- ura_planning_areas_map + tm_shape(dus_facilities_shp) +
    tm_dots(size = 0.3,
          fill = "navy",
          fill.legend = tm_legend(title = "Sports Fields"),
          fill_alpha = 0.5)

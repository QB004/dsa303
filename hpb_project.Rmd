...
#Distance of the nearest food option to each school (Nina)
```{r} 
...
find_nearest_food_options <- function(schools_sf, food_options_sf) {
  ...
  # Calculate pairwise distances using geosphere::distGeo
  distances <- st_distance(schools_sf, food_options_sf)

  # Find the index of the nearest food option for each school
  nearest_indices <- apply(distances, 1, which.min)

  # Extract the distances to the nearest food options
  nearest_distances <- apply(distances, 1, min)

  # Extract the outlet types of the nearest food options
  ...
  #result_sf <- schools_sf %>%
   # mutate(
    ...
    #select(SCHOOL, distance_to_nearest_food_option, geometry)

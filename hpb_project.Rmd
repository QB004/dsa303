...
```{r}
library(tidyverse); library(sf); library(tmap); library(aspace); library(spdep); library(spatialreg); library(corrplot); library(readxl); library(dplyr)
```
...
obesity_data <- 
  ...
  select(-PT_ID.1) %>% 
  st_as_sf(coords = c("longitude","latitude"), crs = 3414) %>% 

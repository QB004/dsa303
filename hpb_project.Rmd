...
knn1_nb_PRI_line <- obesity_pri_sch %>% 
  knearneigh(k = 1) %>% 
  knn2nb(row.names = obesity_pri_sch$PT_ID) %>% # nearest neighbour
  nb2lines(coords = st_geometry(obesity_pri_sch))
knn2_nb_PRI_line <- obesity_pri_sch %>% 
  knearneigh(k = 2) %>% 
  knn2nb(row.names = obesity_pri_sch$PT_ID) %>% # nearest 2 neighbours
  nb2lines(coords = st_geometry(obesity_pri_sch))

knn3_nb_PRI_line <- obesity_pri_sch %>% 
  knearneigh(k = 3) %>% 
  knn2nb(row.names = obesity_pri_sch$PT_ID) %>% # nearest 3 neighbours
  nb2lines(coords = st_geometry(obesity_pri_sch))

knn1_nb_SEC_line <- obesity_sec_sch %>% 
  knearneigh(k = 1) %>% 
  knn2nb(row.names = obesity_sec_sch$PT_ID) %>% # nearest neighbour
  nb2lines(coords = st_geometry(obesity_sec_sch))

knn2_nb_SEC_line <- obesity_sec_sch %>% 
  knearneigh(k = 2) %>% 
  knn2nb(row.names = obesity_sec_sch$PT_ID) %>% # nearest 2 neighbours
  nb2lines(coords = st_geometry(obesity_sec_sch))

knn3_nb_SEC_line <- obesity_sec_sch %>% 
  knearneigh(k = 3) %>% 
  knn2nb(row.names = obesity_sec_sch$PT_ID) %>% # nearest 3 neighbour
  nb2lines(coords = st_geometry(obesity_sec_sch))

tm_shape(knn1_nb_PRI_line) +
  tm_lines(lwd = 2) + pri_sch_map
tm_shape(knn2_nb_PRI_line) +
  tm_lines(lwd = 2) + pri_sch_map
tm_shape(knn3_nb_PRI_line) +
  tm_lines(lwd = 2) + pri_sch_map
tm_shape(knn1_nb_SEC_line) +
  tm_lines(lwd = 2) + sec_sch_map
tm_shape(knn2_nb_SEC_line) +
  tm_lines(lwd = 2) + sec_sch_map
tm_shape(knn3_nb_SEC_line) +
  tm_lines(lwd = 2) + sec_sch_map
```
